...
public abstract class BaseNineGridImageLayout extends ViewGroup {
    ...
    private ArrayList<WeiboPicCateBean> mUrlList = new ArrayList<>();

    public BaseNineGridImageLayout(Context context) {
        super(context);
        init(context);
    }

    public BaseNineGridImageLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.BaseNineImagesLayoutLayout);

        mSpacing = typedArray.getDimension(R.styleable.BaseNineImagesLayoutLayout_spacing, DEFUALT_SPACING);
        typedArray.recycle();
        init(context);
    }
    ...
    public void setUrlList(List<WeiboPicCateBean> urlList) {
        if (urlList == null || getListSize(urlList) == 0) {
            setVisibility(GONE);
            return;
        }
    ...
    private void refresh() {
        ...
        if (size == 1) {
            String url = mUrlList.get(0).getBmiddle().getUrl();
        ...
        for (int i = 0; i < size; i++) {
            String url = mUrlList.get(i).getBmiddle().getUrl();
    ...
    private RatioImageView createImageView(final int i, final String url) {
        ...
        imageView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                onClickImage(imageView, i, url, mUrlList);
            }
        });
        return imageView;
    }
...
//        params.width = width;
    ...
    private int getListSize(List<WeiboPicCateBean> list) {
    ...
    protected abstract void onClickImage(RatioImageView imageView, int position, String url, ArrayList<WeiboPicCateBean> urlList);
